...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError('');
    try { await login(email, password); } catch { setError(t('auth.loginError')); }
  };
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e1b4b 50%, #0f172a 100%)' }}>
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold gradient-text mb-2">{t('auth.loginTitle')}</h1>
          <p className="text-slate-400">Clinical Cognitive Screening Platform</p>
        </div>
        <form onSubmit={handleSubmit} className="card space-y-4">
          ...
          <p className="text-center text-sm text-slate-400">{t('auth.noAccount')} <Link to="/signup" className="text-blue-400 hover:underline">{t('nav.signup')}</Link></p>
        </form>
